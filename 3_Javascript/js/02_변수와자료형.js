...
console.log(foo1); // 전역변수 이기 때문에(블록과 관계없음) 접근 가능
console.log(foo1); // 전역변수 이기 때문에(블록과 관계없음) 접근 가능
// console.log(num1); // ncaught ReferenceError: num1 is not defined
// console.log(num2); // 02_변수와자료형.js:110 Uncaught ReferenceError: num2 is not defined
...
function example2(){ // 블록문(if, for, while 등) 내에서 
...
// 코드 실행 과정에서 특정 시점까지 
// 변수가 접근 불가능한 상태를 의미함.
// Jacascript에서 let/const 키워드로 선언된 변수가 
...
const unserName = "홍길동";
console.log("name : ", unserName, typeof unserName);

const phone = '0102341234';
...
const sumFn = function(a, b) { return a + b}; // 익명합수
